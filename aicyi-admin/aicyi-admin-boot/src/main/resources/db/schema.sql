...
--   2. 主键统一雪花 ID：所有表 id BIGINT 非自增（禁用 AUTO_INCREMENT），
--      由应用层 IdUtils.generateId() 生成后写入（对应 @TableId(type = IdType.INPUT)）。
...
CREATE TABLE IF NOT EXISTS sys_user (
    id                 BIGINT       NOT NULL COMMENT '主键（雪花算法生成，应用层 IdUtils.generateId() 赋值）',
